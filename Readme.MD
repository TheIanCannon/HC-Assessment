...
`git pull` and `git fetch` are often described as being equivalent. In actuality, `git pull` performs `git fetch` followed immediately by `git merge`. We prefer `git pull`, but you may use `git fetch` and `git merge` to ensure you understand the changes you are making to your branch before the merge happens.